...
function bindEvents(): void {
  ...
  // 用户发送消息时，SillyTavern 触发 MESSAGE_SENT（区别于 AI 回复的 MESSAGE_RECEIVED）。
  // 走完整 handleMessageContent 流程：继承上层变量 / 处理可能的标签 / 清理旧变量。
  eventBus.on(eventBus.EVENTS.MESSAGE_SENT, (messageIndex: number) => {
    const rawIdx = messageIndex as unknown;
    const idx = sanitizeMessageIndexForWrite(rawIdx);
    ...
    try {
      ...
      notify.error('用户消息处理失败', errorMessageFromUnknown(e), { category: 'msg' });
    }
  });
