...
- **酒馆助手固定 `@v1.0.0` 的 import**：仓库已配置 [`.github/workflows/sync-jsdelivr.yml`](.github/workflows/sync-jsdelivr.yml)——每次 **push 到 `main`** 会自动把 **`v1.0.0` 标签挪到该提交** 并 **请求 jsDelivr 刷新缓存**，一般 **F5** 即可；若仍像旧脚本，试 **Ctrl+F5**（跳过浏览器自己的缓存）。
